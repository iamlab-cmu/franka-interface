...
namespace franka_ros_interface
{
  ...
  int SharedMemoryHandler::loadSkillParametersIntoSharedMemory(const std::shared_ptr<const franka_interface_msgs::action::ExecuteSkill::Goal> goal)
  {
    ...
    {
      std::cout << "Error with the current_skill_id and new_skill_id. current_skill_id = " << current_skill_id << ", new_skill_id = " << new_skill_id << std::endl;
      return -1;
    }
  ...
  void SharedMemoryHandler::setSkillCancelledFlagInSharedMemory(bool skill_cancelled_flag)
  {
    // Grab the lock of the run_loop_info_mutex_
    boost::interprocess::scoped_lock<boost::interprocess::interprocess_mutex> run_loop_info_lock(*run_loop_info_mutex_);

    // Set skill_cancelled_ in run_loop_process_info_ to the input skill_cancelled_flag
    run_loop_process_info_->set_skill_cancelled(skill_cancelled_flag);
  ...
  franka_interface_msgs::action::ExecuteSkill::Feedback SharedMemoryHandler::getSkillFeedback()
  {
    franka_interface_msgs::action::ExecuteSkill::Feedback feedback;
  ...
  franka_interface_msgs::msg::RobotState SharedMemoryHandler::getRobotState(std::array<double, 144> &robot_frames) {
    franka_interface_msgs::msg::RobotState robot_state;
  ...
  franka_interface_msgs::msg::FrankaInterfaceStatus SharedMemoryHandler::getFrankaInterfaceStatus()
  {
    ...
    franka_interface_msgs::msg::FrankaInterfaceStatus franka_interface_status;
  ...
  franka_interface_msgs::msg::RunLoopProcessInfoState SharedMemoryHandler::getRunLoopProcessInfoState()
  {
    boost::interprocess::scoped_lock<boost::interprocess::interprocess_mutex> run_loop_info_lock(*run_loop_info_mutex_, boost::interprocess::defer_lock);

    franka_interface_msgs::msg::RunLoopProcessInfoState run_loop_process_info_state;
  ...
  void SharedMemoryHandler::loadSensorDataUnprotected(const franka_interface_msgs::msg::SensorData &sensor_data, SensorBufferTypePtr sensor_buffer_ptr) {
    ...
    memcpy(sensor_buffer_ptr + 6, &sensor_data.sensor_data[0], sensor_data_size * sizeof(uint8_t));
  }

  //Adding new function to load sensor data into sensor memory buffer
  void SharedMemoryHandler::loadSensorDataGroupIntoSharedMemory(const franka_interface_msgs::msg::SensorDataGroup::SharedPtr sensor_data_group_ptr) {
    if (sensor_data_group_0_mutex_->try_lock())
    {
      if (sensor_data_group_ptr->has_trajectory_generator_sensor_data) {
        loadSensorDataUnprotected(sensor_data_group_ptr->trajectory_generator_sensor_data, sensor_data_trajectory_generator_buffer_0_);
      }

      if (sensor_data_group_ptr->has_feedback_controller_sensor_data) {
        loadSensorDataUnprotected(sensor_data_group_ptr->feedback_controller_sensor_data, sensor_data_feedback_controller_buffer_0_);
      }

      if (sensor_data_group_ptr->has_termination_handler_sensor_data) {
        loadSensorDataUnprotected(sensor_data_group_ptr->termination_handler_sensor_data, sensor_data_termination_handler_buffer_0_);
      }

      sensor_data_group_0_mutex_->unlock();
    } else {
      std::cout << "Failed to get sensor data group 0 mutex" << std::endl;
    }
  }
  ...
  void SharedMemoryHandler::loadTrajGenParamsUnprotected(const std::shared_ptr<const franka_interface_msgs::action::ExecuteSkill::Goal> goal,
  ...
  void SharedMemoryHandler::loadFeedbackControllerParamsUnprotected(
      const std::shared_ptr<const franka_interface_msgs::action::ExecuteSkill::Goal> goal, int current_free_shared_memory_index)
  {
  ...
  // Requires a lock on the mutex of the designated current_free_shared_memory_index buffer
  void SharedMemoryHandler::loadTerminationParamsUnprotected(const std::shared_ptr<const franka_interface_msgs::action::ExecuteSkill::Goal> goal,
                                                             int current_free_shared_memory_index)
  {
  ...
  // Requires a lock on the mutex of the designated current_free_shared_memory_index buffer
  void SharedMemoryHandler::loadTimerParamsUnprotected(const std::shared_ptr<const franka_interface_msgs::action::ExecuteSkill::Goal> goal,
                                                       int current_free_shared_memory_index)
  {
    if(current_free_shared_memory_index == 0)
    {
