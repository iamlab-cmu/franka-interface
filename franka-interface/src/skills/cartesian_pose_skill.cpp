...
void CartesianPoseSkill::execute_skill_on_franka(run_loop* run_loop,
  ...
  if (internal_feedback_controller->set_cartesian_impedance_) {
    robot->robot_.control(cartesian_pose_callback, franka::ControllerMode::kCartesianImpedance, limit_rate = true);
  } else {
    robot->robot_.control(cartesian_pose_callback, franka::ControllerMode::kJointImpedance, limit_rate = true);
  }
}
