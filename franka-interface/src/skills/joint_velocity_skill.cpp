...
void JointVelocitySkill::execute_skill_on_franka(run_loop* run_loop,
                                                 FrankaRobot* robot,
  ...
  JointVelocityTrajectoryGenerator* joint_velocity_trajectory_generator = dynamic_cast<JointVelocityTrajectoryGenerator*>(traj_generator_);
  ...
  if(joint_velocity_trajectory_generator == nullptr) {
    throw std::bad_cast();
  }
  ...
  std::function<franka::JointVelocities(const franka::RobotState&, franka::Duration)>
    ...
    if (time == 0.0) {
      joint_velocity_trajectory_generator->initialize_trajectory(robot_state, SkillType::JointVelocitySkill);
      try {
        if (lock.try_lock()) {
    ...
    bool done = termination_handler_->should_terminate(robot_state, model_,
                                                                 traj_generator_);
    franka::JointVelocities joint_velocities_desired(joint_velocity_trajectory_generator->get_desired_joint_velocities());
  ...
  if (internal_feedback_controller->set_cartesian_impedance_) {
    robot->robot_.control(joint_velocity_callback, franka::ControllerMode::kCartesianImpedance);
  } else {
