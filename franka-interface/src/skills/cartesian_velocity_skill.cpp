...
void CartesianVelocitySkill::execute_skill_on_franka(run_loop* run_loop,
                                                 FrankaRobot* robot,
  ...
  CartesianVelocityTrajectoryGenerator* cartesian_velocity_trajectory_generator = dynamic_cast<CartesianVelocityTrajectoryGenerator*>(traj_generator_);
  ...
  std::function<franka::CartesianVelocities(const franka::RobotState&, franka::Duration)>
    ...
    if (time == 0.0) {
      cartesian_velocity_trajectory_generator->initialize_trajectory(robot_state, SkillType::CartesianVelocitySkill);
      try {
        if (lock.try_lock()) {
    ...
    bool done = termination_handler_->should_terminate(robot_state, model_,
                                                                 traj_generator_);
    franka::CartesianVelocities cartesian_velocities_desired(cartesian_velocity_trajectory_generator->get_desired_cartesian_velocities());
  ...
  if (internal_feedback_controller->set_cartesian_impedance_) {
    robot->robot_.control(cartesian_velocity_callback, franka::ControllerMode::kCartesianImpedance);
  } else {
    robot->robot_.control(cartesian_velocity_callback, franka::ControllerMode::kJointImpedance);
  }
}
