...
namespace franka_ros_interface
{
  ...
  franka_interface_msgs::RobotState SharedMemoryHandler::getRobotState(std::array<double, 144> &robot_frames) {
    ...
    if (current_robot_state_lock.try_lock()) {
      ...
      RobotStateMessage::Errors current_errors = robot_state_msg.current_errors();
      robot_state.current_errors.joint_position_limits_violation = current_errors.joint_position_limits_violation();
      robot_state.current_errors.cartesian_position_limits_violation = current_errors.cartesian_position_limits_violation();
      robot_state.current_errors.self_collision_avoidance_violation = current_errors.self_collision_avoidance_violation();
      robot_state.current_errors.joint_velocity_violation = current_errors.joint_velocity_violation();
      robot_state.current_errors.cartesian_velocity_violation = current_errors.cartesian_velocity_violation();
      robot_state.current_errors.force_control_safety_violation = current_errors.force_control_safety_violation();
      robot_state.current_errors.joint_reflex = current_errors.joint_reflex();
      robot_state.current_errors.cartesian_reflex = current_errors.cartesian_reflex();
      robot_state.current_errors.max_goal_pose_deviation_violation = current_errors.max_goal_pose_deviation_violation();
      robot_state.current_errors.max_path_pose_deviation_violation = current_errors.max_path_pose_deviation_violation();
      robot_state.current_errors.cartesian_velocity_profile_safety_violation = current_errors.cartesian_velocity_profile_safety_violation();
      robot_state.current_errors.joint_position_motion_generator_start_pose_invalid = current_errors.joint_position_motion_generator_start_pose_invalid();
      robot_state.current_errors.joint_motion_generator_position_limits_violation = current_errors.joint_motion_generator_position_limits_violation();
      robot_state.current_errors.joint_motion_generator_velocity_limits_violation = current_errors.joint_motion_generator_velocity_limits_violation();
      robot_state.current_errors.joint_motion_generator_velocity_discontinuity = current_errors.joint_motion_generator_velocity_discontinuity();
      robot_state.current_errors.joint_motion_generator_acceleration_discontinuity = current_errors.joint_motion_generator_acceleration_discontinuity();
      robot_state.current_errors.cartesian_position_motion_generator_start_pose_invalid = current_errors.cartesian_position_motion_generator_start_pose_invalid();
      robot_state.current_errors.cartesian_motion_generator_elbow_limit_violation = current_errors.cartesian_motion_generator_elbow_limit_violation();
      robot_state.current_errors.cartesian_motion_generator_velocity_limits_violation = current_errors.cartesian_motion_generator_velocity_limits_violation();
      robot_state.current_errors.cartesian_motion_generator_velocity_discontinuity = current_errors.cartesian_motion_generator_velocity_discontinuity();
      robot_state.current_errors.cartesian_motion_generator_acceleration_discontinuity = current_errors.cartesian_motion_generator_acceleration_discontinuity();
      robot_state.current_errors.cartesian_motion_generator_elbow_sign_inconsistent = current_errors.cartesian_motion_generator_elbow_sign_inconsistent();
      robot_state.current_errors.cartesian_motion_generator_start_elbow_invalid = current_errors.cartesian_motion_generator_start_elbow_invalid();
      robot_state.current_errors.cartesian_motion_generator_joint_position_limits_violation = current_errors.cartesian_motion_generator_joint_position_limits_violation();
      robot_state.current_errors.cartesian_motion_generator_joint_velocity_limits_violation = current_errors.cartesian_motion_generator_joint_velocity_limits_violation();
      robot_state.current_errors.cartesian_motion_generator_joint_velocity_discontinuity = current_errors.cartesian_motion_generator_joint_velocity_discontinuity();
      robot_state.current_errors.cartesian_motion_generator_joint_acceleration_discontinuity = current_errors.cartesian_motion_generator_joint_acceleration_discontinuity();
      robot_state.current_errors.cartesian_position_motion_generator_invalid_frame = current_errors.cartesian_position_motion_generator_invalid_frame();
      robot_state.current_errors.force_controller_desired_force_tolerance_violation = current_errors.force_controller_desired_force_tolerance_violation();
      robot_state.current_errors.controller_torque_discontinuity = current_errors.controller_torque_discontinuity();
      robot_state.current_errors.start_elbow_sign_inconsistent = current_errors.start_elbow_sign_inconsistent();
      robot_state.current_errors.communication_constraints_violation = current_errors.communication_constraints_violation();
      robot_state.current_errors.power_limit_violation = current_errors.power_limit_violation();
      robot_state.current_errors.joint_p2p_insufficient_torque_for_planning = current_errors.joint_p2p_insufficient_torque_for_planning();
      robot_state.current_errors.tau_j_range_violation = current_errors.tau_j_range_violation();
      robot_state.current_errors.instability_detected = current_errors.instability_detected();
      robot_state.current_errors.joint_move_in_wrong_direction = current_errors.joint_move_in_wrong_direction();

      RobotStateMessage::Errors last_motion_errors = robot_state_msg.last_motion_errors();
      robot_state.last_motion_errors.joint_position_limits_violation = last_motion_errors.joint_position_limits_violation();
      robot_state.last_motion_errors.cartesian_position_limits_violation = last_motion_errors.cartesian_position_limits_violation();
      robot_state.last_motion_errors.self_collision_avoidance_violation = last_motion_errors.self_collision_avoidance_violation();
      robot_state.last_motion_errors.joint_velocity_violation = last_motion_errors.joint_velocity_violation();
      robot_state.last_motion_errors.cartesian_velocity_violation = last_motion_errors.cartesian_velocity_violation();
      robot_state.last_motion_errors.force_control_safety_violation = last_motion_errors.force_control_safety_violation();
      robot_state.last_motion_errors.joint_reflex = last_motion_errors.joint_reflex();
      robot_state.last_motion_errors.cartesian_reflex = last_motion_errors.cartesian_reflex();
      robot_state.last_motion_errors.max_goal_pose_deviation_violation = last_motion_errors.max_goal_pose_deviation_violation();
      robot_state.last_motion_errors.max_path_pose_deviation_violation = last_motion_errors.max_path_pose_deviation_violation();
      robot_state.last_motion_errors.cartesian_velocity_profile_safety_violation = last_motion_errors.cartesian_velocity_profile_safety_violation();
      robot_state.last_motion_errors.joint_position_motion_generator_start_pose_invalid = last_motion_errors.joint_position_motion_generator_start_pose_invalid();
      robot_state.last_motion_errors.joint_motion_generator_position_limits_violation = last_motion_errors.joint_motion_generator_position_limits_violation();
      robot_state.last_motion_errors.joint_motion_generator_velocity_limits_violation = last_motion_errors.joint_motion_generator_velocity_limits_violation();
      robot_state.last_motion_errors.joint_motion_generator_velocity_discontinuity = last_motion_errors.joint_motion_generator_velocity_discontinuity();
      robot_state.last_motion_errors.joint_motion_generator_acceleration_discontinuity = last_motion_errors.joint_motion_generator_acceleration_discontinuity();
      robot_state.last_motion_errors.cartesian_position_motion_generator_start_pose_invalid = last_motion_errors.cartesian_position_motion_generator_start_pose_invalid();
      robot_state.last_motion_errors.cartesian_motion_generator_elbow_limit_violation = last_motion_errors.cartesian_motion_generator_elbow_limit_violation();
      robot_state.last_motion_errors.cartesian_motion_generator_velocity_limits_violation = last_motion_errors.cartesian_motion_generator_velocity_limits_violation();
      robot_state.last_motion_errors.cartesian_motion_generator_velocity_discontinuity = last_motion_errors.cartesian_motion_generator_velocity_discontinuity();
      robot_state.last_motion_errors.cartesian_motion_generator_acceleration_discontinuity = last_motion_errors.cartesian_motion_generator_acceleration_discontinuity();
      robot_state.last_motion_errors.cartesian_motion_generator_elbow_sign_inconsistent = last_motion_errors.cartesian_motion_generator_elbow_sign_inconsistent();
      robot_state.last_motion_errors.cartesian_motion_generator_start_elbow_invalid = last_motion_errors.cartesian_motion_generator_start_elbow_invalid();
      robot_state.last_motion_errors.cartesian_motion_generator_joint_position_limits_violation = last_motion_errors.cartesian_motion_generator_joint_position_limits_violation();
      robot_state.last_motion_errors.cartesian_motion_generator_joint_velocity_limits_violation = last_motion_errors.cartesian_motion_generator_joint_velocity_limits_violation();
      robot_state.last_motion_errors.cartesian_motion_generator_joint_velocity_discontinuity = last_motion_errors.cartesian_motion_generator_joint_velocity_discontinuity();
      robot_state.last_motion_errors.cartesian_motion_generator_joint_acceleration_discontinuity = last_motion_errors.cartesian_motion_generator_joint_acceleration_discontinuity();
      robot_state.last_motion_errors.cartesian_position_motion_generator_invalid_frame = last_motion_errors.cartesian_position_motion_generator_invalid_frame();
      robot_state.last_motion_errors.force_controller_desired_force_tolerance_violation = last_motion_errors.force_controller_desired_force_tolerance_violation();
      robot_state.last_motion_errors.controller_torque_discontinuity = last_motion_errors.controller_torque_discontinuity();
      robot_state.last_motion_errors.start_elbow_sign_inconsistent = last_motion_errors.start_elbow_sign_inconsistent();
      robot_state.last_motion_errors.communication_constraints_violation = last_motion_errors.communication_constraints_violation();
      robot_state.last_motion_errors.power_limit_violation = last_motion_errors.power_limit_violation();
      robot_state.last_motion_errors.joint_p2p_insufficient_torque_for_planning = last_motion_errors.joint_p2p_insufficient_torque_for_planning();
      robot_state.last_motion_errors.tau_j_range_violation = last_motion_errors.tau_j_range_violation();
      robot_state.last_motion_errors.instability_detected = last_motion_errors.instability_detected();
      robot_state.last_motion_errors.joint_move_in_wrong_direction = last_motion_errors.joint_move_in_wrong_direction();
